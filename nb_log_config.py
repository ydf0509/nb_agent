...
# nb_agent TUI 必需：以下两项设为 None，禁止 nb_log 劫持 sys.stdout（会破坏 Textual 渲染）
...
# nb_agent TUI 必需：禁止 monkey patch print
AUTO_PATCH_PRINT = False
SHOW_PYCHARM_COLOR_SETINGS = False    # TUI 模式不需要
SHOW_NB_LOG_LOGO = False              # TUI 模式不需要
SHOW_IMPORT_NB_LOG_CONFIG_PATH = False  # TUI 模式不需要
